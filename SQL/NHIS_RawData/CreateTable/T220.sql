--create directory T220 as 'D:\NHIS_DATA\06.T220';
...
begin
	...
	loop
	execute immediate(
		'create table NHID_GY20_T2_'||idx||'_ext
			(PERSON_ID	NUMBER(8)
			,KEY_SEQ	VARCHAR2(12)
			,YKIHO_ID	NUMBER(6)
	...
	loop
		execute immediate(
			'create table NHID_GY20_T2_'||idx||'
			as
			select * from NHID_GY20_T2_'||idx||'_ext');
	end loop;

	for idx in 2002..2013
	loop
		execute immediate(
			'select * from NHID_GY20_T2_'||idx);
	end loop;
end;

--drop table NHID_GY20_T1_2004_ext;
--alter table NHID_GY20_T1_2004 rename to NHID_GY20_T1_2004_ext;  
--select * from NHID_GY20_T2_2002;
--select * from NHID_GY20_T2_2013;
